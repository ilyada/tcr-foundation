...
#SBATCH --ntasks=1
#SBATCH --output=joint_full-%j.log

# TCRFoundation joint run launched THROUGH the library: `python -m tcr_foundation.train` wraps the vendored
...
#   sbatch --export=ALL,CONDA_ENV=<env>,EPOCHS=5 tcr_foundation/slurm/joint_full.sh
#   sbatch --partition=short --time=00:30:00 --export=ALL,CONDA_ENV=<env>,SMOKE=1,SAVE_PATH=/tmp/joint_smoke tcr_foundation/slurm/joint_full.sh
#
...
#   CONDA_ENV        - conda environment to activate (required; supplied by the site launcher)
#   CONFIG           - YAML config (default: ../tcr_foundation/tcr_foundation/configs/joint_full.yaml, relative to WORKDIR)
...
CONDA_ENV=${CONDA_ENV:?set CONDA_ENV through sbatch --export}
CONFIG=${CONFIG:-../tcr_foundation/tcr_foundation/configs/joint_full.yaml}
NUM_WORKERS=${NUM_WORKERS:-16}
COMET_PROJECT=${COMET_PROJECT:-tcr_foundation}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"TCR-Foundation | joint cdr123 | OTS+pan+Tanno+Emerson | tiny bf16 | FULL"}
...
echo "=========================================="
echo "TCRFoundation joint cdr123 -- via library (python -m tcr_foundation.train)"
echo "  Repo root:       $REPO"
...
python -m tcr_foundation.train --config "$CONFIG" --num-workers "$NUM_WORKERS" ${EPOCHS:+--epochs "$EPOCHS"} ${BATCH_SIZE:+--batch-size "$BATCH_SIZE"} ${SAVE_PATH:+--save-path "$SAVE_PATH"} --comet-project "$COMET_PROJECT" --experiment-name "$EXPERIMENT_NAME" ${SMOKE:+--smoke} ${RESUME:+--resume}

if [ $? -ne 0 ]; then
    echo "ERROR: joint training failed"
    exit 1
fi

echo "Joint training complete. Foundation saved (see RESOLVED JOINT CONFIG save_path)."
